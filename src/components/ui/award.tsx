  ...
  if (variant === "award") {
    return (
      ...
      >
        ...
        <div className="z-10 px-4 py-5 text-center relative max-w-[170px] mx-auto">

          <h1 className={cn("text-[16px] font-black tracking-tight text-foreground drop-shadow-sm leading-tight break-words")}>{title}</h1>

          <h2 className={cn("text-[12px] font-medium text-muted-foreground leading-snug break-words mt-1")}>{subtitle}</h2>

          {recipient && (
            <p className={cn("text-accent/90 italic text-[11px] mt-0.5 break-words")}>{recipient}</p>
          )}

          <div className={cn("text-accent text-[11px] font-bold mt-1.5")}>{date}</div>
        </div>
      </div>
    )
  }
